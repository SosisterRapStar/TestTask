...
@router.patch("/{category_id}/", status_code=status.HTTP_200_OK, response_model=CategoryForResponse)
async def update_category(updating_category: CategoryForUpdate, service: category_service, category_id: uuid.UUID):
    return await service.update_category(id=category_id, updating_category=update_category)
